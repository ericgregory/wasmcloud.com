...
In our previous tutorials, we started a developer loop with `wash dev` and added two **interfaces** (`wasi:logging` and `wasi:keyvalue`) to our application.

Now we'll learn how to: 

* Explore capabilities available for wasmCloud applications
* Swap out the building blocks that provide key functionality 
* Deploy an application in a local wasmCloud environment

In the process, we'll look under the hood and see how **capabilities** work.

:::info[Prerequisites]
This tutorial assumes you're following directly from the previous steps. Make sure to complete [**Quickstart**](/docs/tour/hello-world.mdx) and [**Plug in Capabilities**](/docs/tour/adding-capabilities.mdx) first.
:::

## Choose a capability

As we learned in the previous tutorial, components can make use of pluggable **capabilities** for common requirements like key-value storage or logging. 
...
Interface packages are named according to the following pattern:

> `namespace:identifier`

The `wasi` namespace tells us that this interface belongs to the [**WebAssembly System Interface (WASI)**](https://wasi.dev/), a group of APIs governed by the WASI Subgroup in the W3C WebAssembly Community Group. 

Interface packages are self-documenting&mdash;for instructions on using the `wasi:keyvalue` interface, we can read through the `.wit` files in the `/wit/deps/keyvalue` folder of our project directory or on GitHub (linked in the Capability Catalog). 

The wasmCloud component template includes common WASI interfaces like `wasi:keyvalue` and `wasi:logging` in the `/wit/deps/` folder. (Note that these interfaces must be explicitly imported in `world.wit` to be available to the component.) 

For more on using interfaces with your native language toolchain and adding new interfaces to an application, see the [Interfaces](/docs/developer/interfaces/creating-an-interface) page in our [Developer Guide](/docs/developer/).
...
When we deploy a wasmCloud application manually, we use an **application manifest** written in YAML, much like a Kubernetes manifest. 
...
We can modify our `wadm.yaml` to include the Redis provider. We'll also need to **configure a link from the component to the provider**.
...
* In the application spec, we define the three major pieces of our app: the `http-component` component, the `kvredis` provider, and the `httpserver` provider. 
* The `kvredis` provider uses the OCI image address we copied previously.
* Since the logging provider is built into wasmCloud as part of its trusted compute base, we don't have to define it at all!
* The `http-component` "imports" the key-value capability, so we define the `kvredis` provider as its target on the `wasi:keyvalue` interface with a [**link**](/docs/concepts/linking-components/).
* The `http-component` "exports" on the `wasi:http` interface, meaning that it exposes a function for calls by another entity. In this case, we define a link from the `httpserver` provider to `http-component`.
...
We can check the status of the app:
