...
On macOS or Linux, you can use [Homebrew](https://brew.sh/) to install `wash`:
...
If you'd like to use a different installation method, see the [**Installation**](/docs/installation) page. 

You can verify that `wash` is installed by running:
...
wasmCloud supports building WebAssembly components from any language that supports the [WASI 0.2 target](/docs/concepts/interfaces#well-known-interfaces), including Go, Rust, TypeScript, Python, and others. 
...
Once you've installed Rust on **macOS, Linux, or Windows**, use `rustup` to add the `wasm32-wasi` target:
...
* [Python 3.10+](https://www.python.org/) and [`pip`](https://pypi.org/project/pip/) v24+ 
* [componentize-py v0.12+](https://github.com/bytecodealliance/componentize-py/tree/main?tab=readme-ov-file#getting-started)

On macOS or Linux, you can use Homebrew to install Python and `pip`:

```shell
brew install python@3.12
```
On Windows, you can use [Chocolatey](https://chocolatey.org/) to install Python and `pip`:

```shell
choco install python3
```

Once you've installed Python and `pip` on macOS, Linux, or Windows, you can use `pip` to install `componentize-py`:
...
The Go code for our component is found in `hello/hello.go`. 

Open the file in your code editor and change the "Hello" message on line 32. You might modify the message to say hello from WebAssembly, your own name, or whatever else you like.
...
bodyWrite.BlockingWriteAndFlush([]uint8("Hello from Wasm!\n")).Unwrap() // [!code ++]
```
...
The Python code for our component is found in `hello/app.py`.

Open the file in your code editor and change the "Hello" message on line 17. You might modify the message to say hello from WebAssembly, your own name, or whatever else you like.
...
outgoingBody.write().blocking_write_and_flush(bytes("Hello from Py!\n", "utf-8")) # [!code --]
outgoingBody.write().blocking_write_and_flush(bytes("Hello from Wasm!\n", "utf-8")) # [!code ++]
...
In this lesson, we installed `wash`, built our first component, and ran our application locally with `wash dev`. 

In our next lesson, we'll **add pluggable, reusable capabilities** like key-value storage to our application.